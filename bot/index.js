...
client.on('ready', async () => {
    ...
    // 🔧 PATCH CRÍTICO — evita erro markedUnread
    try {
        ...
        console.warn('Não foi possível aplicar patch sendSeen');
    }
...
client.on('message', async (message) => {
    if (message.from === GRUPO_ID) {
        ...
        if (/^\d+$/.test(texto)) {
            ...
            // 🛡️ Proteção extra contra crash no sendMessage
            try {
                await client.sendMessage(
                    message.from,
                    '⚠️ Por favor, envie apenas um código numérico (sem letras, espaços ou símbolos).'
                );
            } catch (err) {
